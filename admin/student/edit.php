...
include '../partials/header.php'; // This includes header and sidebar
...
if (isset($_REQUEST['student_id'])) {
    ...
    if ($result->num_rows > 0) {
        // If the student exists, fetch the data
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['student_id'])) {
    ...
    if (empty($errors)) {
        ...
        $stmt->bind_param("sss", $updatedData['first_name'], $updatedData['last_name'], $updatedData['student_id']);
        $stmt->execute();
        $stmt->close();
        $conn->close();
